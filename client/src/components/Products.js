...
function Products() {
    ...
    // axios.get('/products/items').then(console.log("in products")).catch(err=>{console.log("error")})
    ...
    return (
        ...
        <div>
            ...
            {items.map((item) =>
            {
               return (<Item key={item._id} title={item.name} src={item.imgSrc} price={item.price}/>);
            })}
        </div>
    )
}
